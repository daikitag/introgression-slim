...
if ts.num_mutations > 0:
        ...
        individual_df = pd.DataFrame(
            {
                "site_id": [i],
                "effect_size": [
                    mutation.metadata["mutation_list"][0]["selection_coeff"]
                ],
    ...
    trait_average = np.zeros(int(ts.max_root_time))

trait_df = pd.DataFrame(
    {"time": np.arange(int(ts.max_root_time)), "trait_average": trait_average}
)
